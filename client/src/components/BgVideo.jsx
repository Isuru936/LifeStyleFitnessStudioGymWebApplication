    ...
    <div className="relative w-screen h-screen">
      ...
      <div className="absolute top-0 left-0 w-full h-full flex flex-col justify-center items-center text-white">
        ...
        <p className="text-lg text-center mx-auto max-w-lg mt-4">
          At LSFS Gym, we believe that staying fit and strong is not just a
          goal, but a way of life. Our state-of-the-art facilities and expert
          trainers are here to support you on your fitness journey, whether
        ...
        <div className="flex mt-8">
          {developerImages.map((image, index) => (
            <div
              key={index}
              className="developer-image-container w-24 h-24 rounded-full overflow-hidden mx-2"
            >
              <img
                src={image}
                alt={`Developer ${index + 1}`}
                className="developer-image w-full h-full object-cover"
              />
            </div>
          ))}
