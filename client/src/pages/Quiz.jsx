...
const Quiz = () => {
  ...
  return (
    <div className="flex justify-center items-center h-screen w-screen ">
      <img src={backgroundImage} className="absolute w-full h-full object-cover align-center scale-y-[-1] rotate-180 lg:object-center " alt="Background" />
      <div className='bg-black bg-opacity-40 rounded-[20px] border border-stone-800 backdrop-blur-sm justify-center shadow-lg backdrop filter relative p-5'>
        <div className=" p-8 ">
          <div className="mb-12 flex justify-center">
            <h2 className="text-xl text-balance text-center">{questions[currentQuestion].question}</h2>
          </div>
          <div>
    ...
    <div className="flex flex-wrap items-center ">
      {questions[currentQuestion].options.map((option, index) => (
        <button
          key={index}
          type="button"
          className={`button ${selectedOption === option ? 'scale-150 drop-shadow-lg' : 'opacity-100'}flex flex-row items-center m-4 lg:m-6`}
          onClick={() => handleOptionSelect(option)}
        >
          {/* Add images for buttons at the first index */}
          {index === 0 && (
            <img src={maleIcon} className="w-20 h-20 rounded-[10px] " alt="Male Icon" />
          )}
          {index === 1 && (
            <img src={femaleIcon} className="w-20 h-20 rounded-[10px]" alt="Female Icon" />
          )}
          {option}
        </button>
      ))}
    ...
    questions[currentQuestion].options.map((option, index) => (
      <div key={index}>
        <label className="flex justify-center">
          <button
            type="button"
            className={`button ${selectedOption === option ? 'bg-orange-700' : 'bg-orange-600'} rounded-[15px] w-40 h-7 cursor-pointer flex justify-center mb-3`}
            onClick={() => handleOptionSelect(option)}
          >
            {option}
          </button>
          ...
          <div className=" mt-12 flex justify-between">
            <button
              ...
              className="button"
            >
              <img src={arrow} className="w-10 h-10 hover:opacity-70" alt="Left Arrow"/>
            </button>
            <button
              ...
              className="button"
            >
              <img src={arrow} className="w-10 h-10 rotate-180 hover:opacity-70" alt="Right Arrow"/>
            </button>
          </div>
        </div>
      </div>
